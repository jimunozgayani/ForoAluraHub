...
import com.aluracursos.ForoAluraHub.domain.cursos.Curso;
import com.aluracursos.ForoAluraHub.domain.cursos.CursoRepository;
...
public class TopicoService {
    ...
    public DatosListadoTopicos agregarTopico(DatosRegistroTopicos datosRegistroTopico) {
        Curso curso;
        Usuario usuario;
    ...
    public Page<DatosListadoTopicos> getTopicos(Pageable pageable) {
        return topicoRepository.findAllByStatusTrue(pageable).map(DatosListadoTopicos::new);
    }

    public DatosListadoTopicos getTopico(Long id) {
    ...
    @Transactional
    public DatosListadoTopicos actualizaTopico(DatosActualizarTopicos datosActualizarTopico) {
    ...
    public Page<DatosTopicoRespuestas> getRespuestas(Long id, Pageable pageable) {
        if (!topicoRepository.existsById(id) || !topicoRepository.findByStatusById(id)) {
            throw new ErrorDeConsulta("No se halló el topico");
        }
        Page<DatosTopicoRespuestas> datosTopicoRespuestas = topicoRepository.findAllByRespuestas(id, pageable).map(DatosTopicoRespuestas::new);
        return datosTopicoRespuestas;
    }
}